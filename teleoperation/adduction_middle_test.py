...
class Q0OnlyTestNode(Node):
    def __init__(self):
        super().__init__("leap_q4_only_test")
        ...
        self.q4_values = [
            float(v) for v in self.get_parameter("q4_values").value
        ]
        ...
        # Use URDF only to get the true q4 hard limits.
        self.model = pin.buildModelFromUrdf(urdf_path)

        joint_id = self.model.getJointId("4")
        if joint_id == 0:
            raise RuntimeError("Joint '0' not found in URDF.")

        joint = self.model.joints[joint_id]
        self.q4_idx_pin = joint.idx_q
        ...
        self.q4_lower = float(lower[self.q4_idx_pin])
        self.q4_upper = float(upper[self.q4_idx_pin])

        self.get_logger().info(
            f"URDF q4 limits: [{self.q4_lower:.3f}, {self.q4_upper:.3f}] rad"
        )
        ...
        self.get_logger().info(
            f"Captured physical pose. Initial q4={self.q_fixed[4]:.3f} rad"
        )
        self.get_logger().info(
            "ONLY q4 will move. q1..q15 remain frozen."
        )

    def smooth_move_q4(self, q4_start: float, q4_target: float):
        q4_target = float(
            np.clip(q4_target, self.q4_lower, self.q4_upper)
        )
        ...
        self.q_fixed[4] = q4_target
        return q4_target

    def run_test(self):
        current_q4 = float(self.q_fixed[4])

        self.get_logger().info(
            f"q4 test sequence: {self.q4_values}"
        )

        for requested_q4 in self.q4_values:
            if not rclpy.ok():
                break
            ...
            self.get_logger().info(
                f"q4: {current_q4:+.3f} -> {target_q4:+.3f} rad"
            )

            current_q4 = self.smooth_move_q4(
                current_q4,
                target_q4,
            )
        ...
        self.get_logger().info("q4 test completed.")
