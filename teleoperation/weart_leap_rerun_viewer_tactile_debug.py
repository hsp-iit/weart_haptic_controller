...
# ---------------------------------------------------------------------------
# Tactile-normal debug
#
# We intentionally draw ALL six local axes on each fingertip.
# The goal of this viewer is only to answer:
#
#   "Which local axis points out of the physical tactile pad?"
#
# Do NOT use these arrows yet as an optimization target.
# First identify the correct local axis visually.
# ---------------------------------------------------------------------------

TACTILE_DEBUG_FRAMES = {
    "index": "index_tip_head",
    "middle": "middle_tip_head",
    "thumb": "thumb_tip_head",
}

# TACTILE_NORMAL_CANDIDATES = {
...
# Same axis = same color family, negative direction = darker.
TACTILE_AXIS_COLORS = {
    "+X": [240, 70, 70, 255],
    "-X": [130, 35, 35, 255],
    "+Y": [70, 210, 95, 255],
...
class WeartLeapRerunViewerNode(Node):
    def __init__(self) -> None:
        ...
        # Separate Pinocchio model used ONLY for debug geometry.
        # The Rerun URDF tree remains responsible for rendering the hand.
        self.pin_model = pin.buildModelFromUrdf(str(self.urdf_info.path))
        self.pin_data = self.pin_model.createData()
        self.pin_q_index_by_joint_name = {}
        for joint_id, joint_name in enumerate(self.pin_model.names):
            if joint_id == 0:
                continue
            joint = self.pin_model.joints[joint_id]
            if joint.nq == 1:
                self.pin_q_index_by_joint_name[str(joint_name)] = int(joint.idx_q)
        ...
        self._log_tactile_normal_candidates()
    ...
    def _target_q_for_pinocchio(self) -> np.ndarray:
        """Convert the 16-value LEAP vector into Pinocchio's q ordering."""
        q_pin = pin.neutral(self.pin_model)

        for leap_index, joint_name in enumerate(self.urdf_info.joint_name_by_index):
            q_index = self.pin_q_index_by_joint_name.get(str(joint_name))
            if q_index is None:
                continue
            q_pin[q_index] = float(self.target_q[leap_index])

        return q_pin

    def _log_tactile_normal_candidates(self) -> None:
        """
        Draw +X/-X/+Y/-Y/+Z/-Z at the INDEX, MIDDLE and THUMB tip frames.

        The arrows are expressed directly in the same world coordinates used
        by the target URDF overlay.  This makes the test independent of Rerun's
        entity hierarchy: if an arrow visually comes out of the tactile pad,
        that local axis is the candidate sensor normal.
        """
        q_pin = self._target_q_for_pinocchio()

        pin.forwardKinematics(self.pin_model, self.pin_data, q_pin)
        pin.updateFramePlacements(self.pin_model, self.pin_data)

        rr = self.viewer.rr
        recording = self.viewer.recording

        for finger_name, frame_name in TACTILE_DEBUG_FRAMES.items():
            frame_id = self.pin_model.getFrameId(frame_name)
            if frame_id >= len(self.pin_model.frames):
                self.get_logger().warning(
                    f"Tactile debug frame not found in URDF: {frame_name}"
                )
                continue

            tip_pose = self.pin_data.oMf[frame_id]
            origin = np.asarray(tip_pose.translation, dtype=float)
            rotation = np.asarray(tip_pose.rotation, dtype=float)

            # Small point at the reference frame origin.
            rr.log(
                f"/debug/tactile_normals/{finger_name}/origin",
                rr.Points3D(
            ...
            for axis_name, n_local in TACTILE_NORMAL_CANDIDATES.items():
                n_world = rotation @ n_local
                vector = TACTILE_ARROW_LENGTH_M * n_world

                rr.log(
                    f"/debug/tactile_normals/{finger_name}/{axis_name}",
                    rr.Arrows3D(
                        origins=[origin.tolist()],
                        vectors=[vector.tolist()],
                        colors=[TACTILE_AXIS_COLORS[axis_name]],
                        radii=[0.0018],
                        labels=[axis_name],
                    ),
                    recording=recording,
